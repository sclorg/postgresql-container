...
For this, we will assume that you are using the `rhscl/postgresql-96-rhel7` image, available via `postgresql:9.6` imagestream tag in Openshift.
...
available via `postgresql:9.6` imagestream tag in Openshift.
...
```
$ oc new-app postgresql:9.6~https://github.com/sclorg/postgresql-container.git \
  --name new-postgresql \
...
```
$ s2i build --context-dir examples/extending-image/ https://github.com/sclorg/postgresql-container.git rhscl/postgresql-96-rhel7 new-postgresql
```
