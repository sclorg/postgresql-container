...
chroot "${HOST}" /usr/bin/docker create -v "${data_dir}:/var/lib/pgsql/data:Z" --name "${NAME}" ${OPT2} "${IMAGE}" ${OPT3}

# Create and enable systemd unit file for the service
sed -e "s/TEMPLATE/${NAME}/g" /usr/share/container-layer/postgresql/atomic/template.service > "${HOST}/etc/systemd/system/${service_name}.service"
